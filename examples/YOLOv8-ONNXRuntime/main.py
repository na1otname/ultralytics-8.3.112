...
if __name__ == "__main__":
    ...
    detection = YOLOv8(r"F:\work\Porject\SLS\model\detection\SLS_BJ_HandPhone_Detect_v8n_20260401\weights/best.onnx",
                       "F:/data/compare/SaveImages/2025_03_26_10_16_42_757.jpg", args.conf_thres, args.iou_thres)

    # 初始化统计变量
    total_time = 0.0
    frame_times = []
    min_duration = float('inf')  # 正确初始化最小值
    max_duration = 0.0

    # 主测试循环
    print("\nStarting benchmark...")
    test_iterations = 1000 # 测试1000次或所有图像(取较小值)
    ...
    # Perform object detection and obtain the output image
        ...
        # 更新最小/最大时间
        min_duration = min(min_duration, frame_time)
        max_duration = max(max_duration, frame_time)

        # 进度报告
        if (i + 1) % 100 == 0:
            current_fps = (i + 1) / total_time
            print(f"Completed {i + 1}/{test_iterations} iterations | Current FPS: {current_fps:.2f}")

    # 计算结果
    avg_time = total_time / test_iterations
    avg_fps = test_iterations / total_time
    ...
    print(f"Minimum inference:     {min_duration * 1000:.2f} ms")
    print(f"Maximum inference:     {max_duration * 1000:.2f} ms")
